...
Usage:
  ...
  python run_qa.py --model claude-sonnet-4-6
"""
import argparse
import base64
import json
...
def ask_with_pdf(client, pdf_path: Path, question: str, model: str) -> tuple[str, int, int]:
...
def ask_with_markdown(client, md_text: str, question: str, model: str) -> tuple[str, int, int]:
    prompt = f"<document>\n{md_text}\n</document>\n\n{question}"
    resp = client.messages.create(
        model=model,
        max_tokens=MAX_TOKENS,
...
def main():
    ...
    for entry in catalog:
        ...
        if not md_path.exists():
            print(f"SKIP {entry['id']}: markdown not converted"); continue
        ...
        print(f"\n{'='*60}")
        print(f"  {entry['name']}")
        print(f"  PDF: {pdf_path.stat().st_size//1024} KB  MD: {md_path.stat().st_size//1024} KB")
        print(f"{'='*60}")

        for i, question in enumerate(questions, 1):
            print(f"\n  Q{i}: {question[:70]}…" if len(question) > 70 else f"\n  Q{i}: {question}")

            for approach, func, arg in [
                ("pdf",      ask_with_pdf,      pdf_path),
                ("markdown", ask_with_markdown, md_text),
            ]:
                out_file = out_dir / f"q{i:02d}_{approach}.txt"
                meta_file = out_dir / f"q{i:02d}_{approach}.meta.json"

                if out_file.exists():
                    print(f"    {approach:8}: SKIP (already done)")
                    continue

                try:
                    answer, in_tok, out_tok = func(client, arg, question, args.model)
                    ...
                    print(f"    {approach:8}: {in_tok:,} in / {out_tok:,} out tokens")
                    time.sleep(0.5)  # polite rate limiting
                except anthropic.RateLimitError:
                    print(f"    {approach:8}: rate limited — waiting 30s…")
                    time.sleep(30)
                except Exception as e:
                    print(f"    {approach:8}: ERROR: {e}")
